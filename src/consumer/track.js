...
export const deleteTrack = (runnerId, trackId) => {
    return firestoreClient.collection("runners").doc(runnerId).collection("tracks").doc(trackId).delete();
};

export const updateTrackLevel = (runnerId, trackId, level) => {
    return firestoreClient.collection("runners").doc(runnerId).collection("tracks").doc(trackId).update({
        level: level,
        date: Date.now(),
    });
};
...
export const getTracksByRunner = (runnerId) => {
    return firestoreClient
        .collection("runners")
        .doc(runnerId)
        .collection("tracks")
...
export const deleteResourceFromCertifiedTrack = async (docName) => {

    const storage = firebaseClient.storage();
    const fileRef = ref(storage, `documentos/${docName}`);
