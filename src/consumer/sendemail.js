...
export function sendNotifyNewMember(
    ...
    const sendemail =
        ...
        "&replyTo=" +
        email;
    return fetch(sendemail).then((res) => res.json());
}
...
export function sendNotifyResponseCertified(journeyId,
    email,
    memberName,
    leaderEmail,
    certifiedName, link,  filename, attachment){
        console.log(link);
        const sendeamil =
        "/api/sendemail/?email=" +
        leaderEmail +
        "&template=new-response-to-certified&name=" +
        memberName +
        "&certifiedName=" +
        certifiedName +
        "&replyTo=" +
